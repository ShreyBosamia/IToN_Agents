...
* **Security:** CI runs with least-privilege; secrets (e.g., OPENAI\_API\_KEY) are not required for unit tests. Avoid real network in CI; document that Playwright installs are explicit (npx playwright install) and not required on every CI run.  
